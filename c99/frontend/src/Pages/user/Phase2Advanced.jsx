...
const parseQuestions = (data) => {
  ...
  const questions = questionBlocks.map((block) => {
    ...
    if (questionMatch && optionsMatch && answerMatch) {
      const options = optionsMatch[0]
        .split("\n")
        .map((option) => option.replace(/^[A-D]\.\s*/, "").trim());
      return {
        question_text: questionMatch[1].trim(),
        options,
        correct_answer: options[answerMatch[1].charCodeAt(0) - 65], // Map 'A' to options[0], 'B' to options[1], etc.
      };
    }
    return null;
  });
  ...
  const fetchQuestions = async () => {
    ...
    try {
      const response = await axios.post(
        ...
        { core_concept: "OOPS", level: "advanced" }
      );
