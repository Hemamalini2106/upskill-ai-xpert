...
const Advance = () => {
  ...
  useEffect(() => {
    fetchQuestions();
  }, [currentLevel]); // Fetch questions when level changes

  const fetchQuestions = async () => {
    setLoading(true);
    try {
      const topic = topics[currentLevel - 1]; // Get the topic for the current level
      const response = await axios.post(
        "http://127.0.0.1:8000/generate_questions",
        {
          core_concept: topic, // Use the topic dynamically
          level: `level_${currentLevel}`, // Dynamically set level
        }
      );
  ...
  const goToNextLevel = () => {
    if (currentLevel === 10) {
      navigate("/phase2basic");
      return;
    }
    setCurrentLevel((prev) => prev + 1); // Move to next level
    setUserAnswers({});
    setSubmitted(false);
    setScore(null);
    setShowModal(false);
